...
function normalizeLessonSelectedStudentFilterV9812() {
  ...
  const ok = Array.from(select.options || []).some(opt => opt.value === selected.value && opt.value);
  if (!ok) {
    select.value = "";
    return "";
  }
...
function ensureCompletedImportButtonV88(){
  ...
  if(btn.dataset.boundCompletedImportV9813==="true")return;
  btn.dataset.boundCompletedImportV9813="true";
  btn.onclick=()=>{input.click();};
...
async function importCompletedLessonExcelV88(file){
  if (typeof importCompletedLessonExcelV886 === "function") {
    return importCompletedLessonExcelV886(file);
  }
