// === v9.6.2 teacher duty declaration template export ===
// 导出给老师填写的勤务申报模板。
...
(function () {
  ...
  function addWorksheetContent(wb, rows) {
    ...
    ws.columns = [
      { key: "date", width: 18 },
      { key: "content1", width: 32 },
      { key: "content2", width: 32 },
      { key: "start", width: 12 },
      { key: "end", width: 12 },
      { key: "hours", width: 10 },
    ...
    setMergeValue(ws, "A1:I1", "勤务申报表（讲师填写用）", {
    ...
    setMergeValue(ws, "G2:I2", "日元银行 / 支付宝 / 微信", { align: "center" });
    ...
    setMergeValue(ws, "A3:I3", "※ 本表仅用于勤务时间、交通费、教室费和支付方式申报。请勿修改系统已填写的日期、工作内容、开始时间、结束时间、时长。", {
      align: "left", size: 10,
    });

    const headers = ["日期及星期", "工作内容", "", "开始时间", "结束时间", "时长", "当日交通费", "当日教室费", "备注"];
    ws.getRow(4).values = headers;
    ws.mergeCells("B4:C4");
    styleRange(ws, "A4:I4", { fill: COLORS.green, bold: true, align: "center" });
    ...
    for (let i = 0; i < 31; i++) {
      ...
      if (row) {
        ...
        ws.getCell(rowNo, 2).value = workContent(row);
        ws.mergeCells(rowNo, 2, rowNo, 3);
        ws.getCell(rowNo, 4).value = formatTime(row.start_time);
        ws.getCell(rowNo, 5).value = formatTime(row.end_time);
        ws.getCell(rowNo, 6).value = formatHours(row.wage?.hours);
      } else {
        ws.getCell(rowNo, 6).value = 0;
      }
      ws.getCell(rowNo, 7).value = 0;
      ws.getCell(rowNo, 8).value = 0;

      for (let c = 1; c <= 9; c++) {
        const align = c === 2 ? "left" : (c >= 6 && c <= 8 ? "right" : "center");
        styleCell(ws.getCell(rowNo, c), { align });
      }
      ws.getRow(rowNo).height = row ? 28 : 18;
    }

    // 合计行：只填充 A:I 表格范围，不影响右侧空白列。
    const totalRow = 36;
    setMergeValue(ws, `A${totalRow}:E${totalRow}`, "合计", { fill: COLORS.orange, bold: true, align: "left" });
    ws.getCell(`F${totalRow}`).value = { formula: `SUM(F5:F35)` };
    ws.getCell(`G${totalRow}`).value = { formula: `SUM(G5:G35)` };
    ws.getCell(`H${totalRow}`).value = { formula: `SUM(H5:H35)` };
    ws.getCell(`I${totalRow}`).value = "";
    styleRange(ws, `F${totalRow}:I${totalRow}`, { fill: COLORS.orange, bold: true, align: "right" });

    setMergeValue(ws, "A37:I37", "日元支付（银行振込）", { fill: COLORS.orange, bold: true, align: "left" });

    ws.getCell("A38").value = "銀行名";
    ws.getCell("B38").value = "支店番号";
    ws.getCell("C38").value = "支店名";
    ws.getCell("D38").value = "口座番号";
    setMergeValue(ws, "E38:F38", "名義", { fill: COLORS.green, bold: true, align: "center" });
    setMergeValue(ws, "G38:I38", "备注", { fill: COLORS.green, bold: true, align: "center" });
    ["A38", "B38", "C38", "D38"].forEach(addr => styleCell(ws.getCell(addr), { fill: COLORS.green, bold: true, align: "center" }));

    ws.getRow(39).values = ["", "", "", "", "", "", "", "", ""];
    ws.mergeCells("E39:F39");
    ws.mergeCells("G39:I39");
    styleRange(ws, "A39:I39", { align: "center" });

    setMergeValue(ws, "A40:I40", "人民币支付", { fill: COLORS.orange, bold: true, align: "left" });

    ws.getCell("A41").value = "支付宝";
    setMergeValue(ws, "B41:D41", "微信", { fill: COLORS.green, bold: true, align: "center" });
    setMergeValue(ws, "E41:I41", "备注", { fill: COLORS.green, bold: true, align: "center" });
    styleCell(ws.getCell("A41"), { fill: COLORS.green, bold: true, align: "center" });

    ws.getRow(42).values = ["", "", "", "", "", "", "", "", ""];
    ws.mergeCells("B42:D42");
    ws.mergeCells("E42:I42");
    styleRange(ws, "A42:I42", { align: "center" });
