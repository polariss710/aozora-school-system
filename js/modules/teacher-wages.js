// === v9.1.3 teacher wage settlement basic module ===
(function () {
  function n(v){ const x = Number(v || 0); return Number.isFinite(x) ? x : 0; }
  function currentMonth(){ return new Date().toISOString().slice(0, 7); }
  function fmtHours(v){ const x = Number(v || 0); return Number.isInteger(x) ? String(x) : String(Math.round(x * 100) / 100); }
  ...
  function parseTime(v){ const m = String(v || "").trim().match(/^(\d{1,2}):(\d{1,2})$/); return m ? Number(m[1]) * 60 + Number(m[2]) : null; }
  ...
  function payHours(minutes){ return Math.floor(n(minutes) / 30) * 0.5; }
  ...
  function rows(){
    ...
    return (state.lessonRecords || []).filter(r => r.year_month === month && (!teacherId || r.teacher_id === teacherId) && isTarget(r))
      .sort((a,b) => teacherName(a).localeCompare(teacherName(b),"zh-Hans-CN") || subjectName(a).localeCompare(subjectName(b),"zh-Hans-CN") || String(a.lesson_date||"").localeCompare(String(b.lesson_date||"")) || String(a.start_time||"").localeCompare(String(b.start_time||"")));
  }
  function summarize(list){
    const map = new Map();
    list.forEach(r => {
      const key = `${r.teacher_id}|${r.subject_id}|${hourlyRate(r)}|${currency(r)}`;
      if (!map.has(key)) map.set(key, { teacher: teacherName(r), subject: subjectName(r), rate: hourlyRate(r), currency: currency(r), minutes: 0, count: 0 });
      const x = map.get(key);
      x.minutes += actualMinutes(r);
      x.count += 1;
    });
    return Array.from(map.values()).map(x => ({...x, hours: payHours(x.minutes), amount: payHours(x.minutes) * n(x.rate)}));
  }
  function render(){
    fillFilters();
    const list = rows();
    ...
    if (sumBody) sumBody.innerHTML = summary.length ? summary.map(x => `<tr><td>${esc(x.teacher)}</td><td>${esc(x.subject)}</td><td>${Math.round(x.minutes)}</td><td>${fmtHours(x.hours)}H</td><td>${fmtAmount(x.rate,x.currency)}</td><td><strong>${fmtAmount(x.amount,x.currency)}</strong></td><td>${x.count}</td></tr>`).join("") : `<tr><td colspan="7" class="empty-row">当前条件下没有可计算工资的实际课时</td></tr>`;
    const detail = document.getElementById("teacherWageDetailTable");
    if (detail) detail.innerHTML = list.length ? list.map(r => `<tr><td>${esc(displayRecordDate(r.lesson_date||""))}</td><td>${esc(teacherName(r))}</td><td>${esc(studentName(r))}</td><td>${esc(subjectName(r))}</td><td>${esc([r.start_time,r.end_time].filter(Boolean).join(" - ") || "时间未定")}</td><td>${Math.round(actualMinutes(r))}</td><td>${fmtHours(r.duration_hours)}H</td><td>${badge(lessonStatusLabel(r.status),"")}</td><td>${esc(short(r.lesson_content || r.note, 32))}</td></tr>`).join("") : `<tr><td colspan="9" class="empty-row">当前条件下没有课时明细</td></tr>`;
  }
  function bind(){
    fillFilters();
    ...
    document.getElementById("teacherWageClearFilter")?.addEventListener("click", () => {
      ...
      if (m) m.value = currentMonth(); if (t) t.value = ""; render();
    });
    render();
  }
  const oldSwitch = typeof switchPage === "function" ? switchPage : null;
  if (oldSwitch) window.switchPage = function(page){
    oldSwitch(page);
  ...
  if (oldRenderAll) window.renderAll = function(){ oldRenderAll(); if (document.getElementById("page-teacher-wages")?.classList.contains("active")) setTimeout(bind, 0); };
  document.addEventListener("DOMContentLoaded", () => setTimeout(() => { if (document.getElementById("page-teacher-wages")?.classList.contains("active")) bind(); }, 1000));
  window.SchoolTeacherWagesModule = { version: "9.1.3", render, summarize, targetLessons: rows, payHoursFromMinutes: payHours };
})();
